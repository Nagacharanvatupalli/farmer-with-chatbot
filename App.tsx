      ...
      <div className="w-full max-w-[1440px] mx-auto px-8 flex justify-between items-center">
        ...
        <div className="flex items-center gap-10">
          ...
          <div className="flex items-center gap-5">
            <div ref={langRef} className={`relative p-1 rounded-xl flex items-center gap-1 border ${currentView === 'home' && window.scrollY <= 20 ? 'bg-white/10 border-white/20' : 'bg-slate-100 border-slate-200'}`}>
              <Globe size={14} className={`${currentView === 'home' && window.scrollY <= 20 ? 'text-white/60' : 'text-slate-400'} mx-1`} />
              <button
                className={`flex items-center gap-2 text-[10px] font-black uppercase pr-2 pl-1 py-1 bg-transparent outline-none ${currentView === 'home' && window.scrollY <= 20 ? 'text-white' : 'text-slate-900'}`}
              ...
              {langOpen && (
                ...
                >
                  {languages.map((l, i) => (
                    ...
                    >
                      <span className={`${currentView === 'home' && window.scrollY <= 20 ? (l.code === lang ? 'text-green-200 font-black' : 'text-white') : (l.code === lang ? 'text-green-700 font-black' : 'text-slate-700')}`}>{l.name}</span>
                    </div>
                  ))}
                </div>
              )}
